---
title: Java基础01
date: 2024-03-26 21:21:57
permalink: /pages/0589be/
categories:
  - java
  - javaSE
tags:
...
![运行在 Java 虚拟机之上的编程语言](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/java-virtual-machine-program-language-os.png)
...
![JDK 包含 JRE](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/jdk-include-jre.png)
...
![Java程序转变为机器代码的过程](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/java-code-to-machine-code.png)

我们需要格外注意的是 `.class->机器码` 这一步。在这一步 JVM 类加载器首先加载字节码文件，然后通过解释器逐行解释执行，这种方式的执行速度会相对比较慢。而且，有些方法和代码块是经常需要被调用的(也就是所谓的热点代码)，所以后面引进了 **JIT（Just in Time Compilation）** 编译器，而 JIT 属于运行时编译。当 JIT 编译器完成第一次编译后，其会将字节码对应的机器码保存下来，下次可以直接使用。而我们知道，机器码的运行效率肯定是高于 Java 解释器的。这也解释了我们为什么经常会说 **Java 是编译与解释共存的语言** 。

![Java程序转变为机器代码的过程](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/java-code-to-machine-code-with-jit.png)

> HotSpot 采用了惰性评估(Lazy Evaluation)的做法，根据二八定律，消耗大部分系统资源的只有那一小部分的代码（热点代码），而这也就是 JIT 所需要编译的部分。JVM 会根据代码每次被执行的情况收集信息并相应地做出一些优化，因此执行的次数越多，它的速度就越快。

JDK、JRE、JVM、JIT 这四者的关系如下图所示。

![JDK、JRE、JVM、JIT 这四者的关系](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/jdk-jre-jvm-jit.png)

下面这张图是 JVM 的大致结构模型。

![JVM 的大致结构模型](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/jvm-rough-structure-model.png)
...
![编译型语言和解释型语言](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/compiled-and-interpreted-languages.png)
...
![Java 注释类型总结](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/java-annotation-types.png)
...
移位运算符在各种框架以及 JDK 自身的源码中使用还是挺广泛的，
...
![Java 移位运算符总结](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/shift-operator.png)
...
![基本类型 vs 包装类型](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/primitive-type-vs-packaging-type.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/up-1ae0425ce8646adfb768b5374951eeb820d.png)
...
**我们发现装箱其实就是调用了 包装类的`valueOf()`方法，拆箱其实就是调用了 `xxxValue()`方法。**
...
`BigDecimal` 可以实现对浮点数的运算，不会造成精度丢失。通常情况下，大部分需要浮点数精确运算结果的业务场景（比如涉及到钱的场景）都是**通过 `BigDecimal` 来做的。**
...
![成员变量 vs 局部变量](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/member-var-vs-local-var.png)
...
> 重载就是**同样的一个方法能够根据输入数据的不同，做出不同的处理**
>
