...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220419155214614.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220419155514392.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220419154632551.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220419155802288.png)
...
**字节流和字节缓冲流的性能差别**主要体现在我们使用两者的时候都是调用 `write(int b)` 和 `read()` 这两个一次只读取一个字节的方法的时候。由于**字节缓冲流内部有缓冲区（字节数组），因此，字节缓冲流会先将读取到的字节存放在缓存区，大幅减少 IO 次数**，提高读取效率。
...
这里要介绍的随机访问流指的是**支持随意跳转到文件的任意位置进行读写的 `RandomAccessFile` 。**
...
`RandomAccessFile` 中有**一个文件指针用来表示下一个将要被写入或者读取的字节所处的位置。**我们可以通过 `RandomAccessFile` 的 **`seek(long pos)` 方法来设置文件指针的偏移量（距文件开头 `pos` 个字节处）。如果想要获取文件指针当前的位置的话，可以使用 `getFilePointer()` 方法。**
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220421162050158.png)
...
**`RandomAccessFile` 比较常见的一个应用**就是实现大文件的 **断点续传** 。何谓断点续传？**简单来说就是上传文件中途暂停或失败（比如遇到网络问题）之后，不需要重新上传，只需要上传那些未成功上传的文件分片即可。分片（先将文件切分成多个文件分片）上传是断点续传的基础。**

`RandomAccessFile` 可以帮助我们合并文件分片，示例代码如下：

![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/20210609164749122.png)

我在[《Java 面试指北》](https://javaguide.cn/zhuanlan/java-mian-shi-zhi-bei.html)中详细介绍了大文件的上传问题。

![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20220428104115362.png)
