...
- **实参（实际参数，Arguments）**：**用于传递**给函数/方法的参数，**必须有确定的值。**
- **形参（形式参数，Parameters）**：**用于定义**函数/方法，接收实参，**不需要有确定的值。**
...
- **引用传递**：方法接收的**直接是实参所引用的对象在堆中的地址，不会创建副本，对形参的修改将影响到实参。**
