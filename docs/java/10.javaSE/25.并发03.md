---
title: 并发03
date: 2024-04-08 15:15:36
permalink: /pages/05a494/
categories:
  - java
  - javaSE
tags:
...
**JDK 中自带的`ThreadLocal`类**正是为了解决这样的问题。 **`ThreadLocal`类主要解决的就是让每个线程绑定自己的值，可以将`ThreadLocal`类形象的比喻成存放数据的盒子，盒子中可以存储每个线程的私有数据。**

如果你创建了一个`ThreadLocal`变量，那么访问这个变量的每个线程都会有这个变量的本地副本，这也是`ThreadLocal`变量名的由来。他们可以使**用 `get()` 和 `set()` 方法来获取默认值或将其值更改为当前线程所存的副本的值，从而避免了线程安全问题。**
...
![ThreadLocal 数据结构](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/threadlocal-data-structure.png)

`ThreadLocalMap`是`ThreadLocal`的静态内部类。

![ThreadLocal内部类](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/thread-local-inner-class.png)
...
**池化技术**想必大家已经屡见不鲜了，线程池、数据库连接池、HTTP 连接池等等都是对这个思想的应用**。池化技术的思想主要是为了减少每次获取资源的消耗，提高对资源的利用率。**
...
![image-20240409223020683](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20240409223020683.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/executors-inner-threadpool.png)
...
![image-20240409221122520](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20240409221122520.png)
...
![image-20240409221152957](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20240409221152957.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/threadpoolexecutor-methods.png)
...
![动态配置线程池参数最终效果](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/meituan-dynamically-configuring-thread-pool-parameters.png)
...
![ThreadPoolExecutor构造函数](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/common-parameters-of-threadpool-workqueue.jpg)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/completablefuture-class-diagram.jpg)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/image-20210902093026059.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/AQS.png)
...
![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/40cb932a64694262993907ebda6a0bfe~tplv-k3u1fbpfcp-zoom-1.png)

AQS(`AbstractQueuedSynchronizer`)的核心原理图（图源[Java 并发之 AQS 详解](https://www.cnblogs.com/waterystone/p/4920797.html)）如下：

![](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/CLH.png)
