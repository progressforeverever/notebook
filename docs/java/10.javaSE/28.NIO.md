---
title: NIO
date: 2024-04-09 21:51:44
permalink: /pages/ab5c9f/
categories:
  - java
  - javaSE
tags:
...
![BIO、NIO 和 AIO 对比](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/bio-aio-nio.png)
...
![Buffer、Channel和Selector三者之间的关系](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/channel-buffer-selector.png)
...
![Buffer 的子类](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/buffer-subclasses.png)
...
![position 、limit 和 capacity 之前的关系](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/JavaNIOBuffer.png)

![position 、limit 和 capacity 之前的关系](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/NIOBufferClassAttributes.png)
...
![capacity、limit和position每一阶段的变化](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/NIOBufferClassAttributesDataChanges.png)
...
**BIO 中的流是单向的，分为各种 `InputStream`（输入流）和 `OutputStream`（输出流），数据只是在一个方向上传输**。
...
![Channel 和 Buffer之间的关系](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/channel-buffer.png)

**另外，因为 Channel 是全双工的**，所以它可以比流更好地映射底层操作系统的 API。特别是在 UNIX 网络编程模型中，底层操作系统的通道都是全双工的，同时支持读写操作。

`Channel` 的子类如下图所示。

![Channel 的子类](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/channel-subclasses.png)
...
![Channel继承关系图](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/channel-inheritance-relationship.png)
...
![Selector 选择器工作示意图](https://czynotebook.oss-cn-beijing.aliyuncs.com/notebook/selector-channel-selectionkey.png)
...
零拷贝是提升 IO 操作性能的一个常用手段，像 ActiveMQ、Kafka 、RocketMQ、QMQ、Netty 等顶级开源项目都用到了**零拷贝。**

零拷贝是指计算机**执行 IO 操作时，CPU 不需要将数据从一个存储区域复制到另一个存储区域**，从而可以减少上下文切换以及 CPU 的拷贝时间。也就是说，**零拷贝主要解决操作系统在处理 I/O 操作时频繁复制数据的问题。零拷贝的常见实现技术有： `mmap+write`、`sendfile`和 `sendfile + DMA gather copy` 。**
